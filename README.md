...
FANZA同人・らぶカルBL・らぶカルTLに対応したアフィリエイト商品サイトです。[PinkClub-FANZA](https://github.com/TaniyanR/PinkClub-FANZA) を基盤に、対象商品とAPI取得先を分離しています。
...
3区分をcron実行ごとに巡回し、取得位置を区分別に保持します。
...
| --- | --- | --- | --- |
| FANZA | doujin | doujin | 同人 |
| FANZA | doujin | bl | らぶカルBL |
| FANZA | doujin | tl | らぶカルTL |
...
```bash
php /path/to/PinkClub-Dojin/scripts/auto_import.php
```
